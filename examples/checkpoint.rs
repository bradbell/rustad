...
fn no_ad_derivative() {
    ...
    let opt_vec        = vec![ ["name",  "example"], ["trace", "false"  ] ];
    let checkpoint_id  = register_checkpoint(f, &directions, &opt_vec);
    //
    // trace, opt_vec
    let trace     = false;
    ...
    let x   : Vec<V> = vec![ V::from(1.0) , V::from(2.0) ];
    let (_, ax)      = start_recording(None, x);
    let (ay, _av)    = g.forward_var_ad(None, ax, &opt_vec);
    let h            = stop_recording(ay);
    let x   : Vec<V> = vec![ V::from(3.0) , V::from(4.0) ];
    let (y, _v)      = h.forward_var_value(None, x.clone(), &opt_vec);
    assert_eq!( y[0], x[0]*x[0] + x[1]*x[1] );
}
//
...
fn forward_ad_derivative() {
    ...
    let (_, ax)      = start_recording(None, x);
    let (_ay, av)    = g.forward_var_ad(None, ax, &opt_vec);
    let ady          = g.forward_der_ad(None, &av , adx, &opt_vec);
    let h            = stop_recording(ady);
    let x   : Vec<V> = vec![ V::from(7.0) , V::from(8.0) ];
    let (dy, _v)     = h.forward_var_value(None, x.clone(), &opt_vec);
    assert_eq!( dy[0], V::from(2.0) * ( x[0]*dx[0] + x[1]*dx[1] ) );
}
//
...
fn reverse_ad_derivative() {
    ...
    let (_, ax)      = start_recording(None, x);
    let (_ay, av)    = g.forward_var_ad(None, ax, &opt_vec);
    let adx          = g.reverse_der_ad(None, &av , ady, &opt_vec);
    let h            = stop_recording(adx);
    let x   : Vec<V> = vec![ V::from(7.0) , V::from(8.0) ];
    let (dx, _v)     = h.forward_var_value(None, x.clone(), &opt_vec);
    assert_eq!( dx[0], V::from(2.0) * x[0] * dy[0] );
    assert_eq!( dx[1], V::from(2.0) * x[1] * dy[0] );
}
//
