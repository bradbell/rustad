...
// ---------------------------------------------------------------------------
// forward_0_mul_cv_fn
/// [ForwardZeroBinary] were op is *, left is cpmstamt, right is variable.
...
pub(crate) fn set_op_info( op_info_vec : &mut Vec<OpInfo> ) {
    op_info_vec[MUL_CV_OP] =
        OpInfo{ name : "mul_cv".to_string() , forward_0 : forward_0_mul_cv_fn };
    op_info_vec[MUL_VC_OP] =
        OpInfo{ name : "mul_vc".to_string() , forward_0 : forward_0_mul_vc_fn };
    op_info_vec[MUL_VV_OP] =
        OpInfo{ name : "mul_vv".to_string() , forward_0 : forward_0_mul_vv_fn };
}
