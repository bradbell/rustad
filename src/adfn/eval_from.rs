...
// ----------------------------------------------------------------------------
// eval_from_value!
/// Convert from the value type to the evaluation type.
///
/// ```syntax
///     let eval_value = eval_from_value(suffix, V, value)
/// ```
///
/// * suffix : If this is `value`, the evaluation type is V.
/// Otherwise suffix must be `ad` and the evaluation type is `AD<V>`
///
/// * V : This is V in [doc_generic_v].
///
/// * value : This is the V value being converted.
///
/// * eval_value : This is the converted value; If suffix is value,
/// it has type V. Otherwise, it has type `AD<V>` .
///
...
macro_rules! eval_from_value {
    //
    (value, $V:ident, $value:expr) => {{
        $value
    }};
    (ad,    $V:ident, $value:expr) => {{
        crate::ad::ad_from_value( $value )
    }};
}
pub(crate) use eval_from_value;
