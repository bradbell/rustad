...
// ---------------------------------------------------------------------------
// record_name_ca
/// Record on binary where left is `V` and right is `AD<V>` .
///
/// We use _ca when left is known to be constant and right is an AD object.
/// We do not use _va to avoid confusion between values and variables.
pub(crate) fn record_ca<V> (
    tape     : &mut Tape<V> ,
    lhs      : &V           ,
    rhs      : &AD<V>       ,
    op_id_pp : u8           ,
...
ad_binary_op!(Div);
// ---------------------------------------------------------------------------
/// Implement FBinary for `AD<V>` and `&AD<V>`
///
/// # Example
/// ```
/// use rustad::{
...
macro_rules! impl_value_binary_ad{ ( $V:ty ) => {
    impl crate::FBinary< &AD<$V> > for &$V
    where
        $V  : crate::FConst,
        for<'a> &'a $V : crate::FUnary<Output=$V> ,
    {   type Output = AD<$V>;
        #[doc= " see [doc_f_binary_ad](crate::ad::binary::doc_f_binary_ad)" ]
        fn powf(self , rhs : &AD<$V>) -> AD<$V> {
    ...
    impl crate::FBinary< AD<$V> > for $V
    where
        for <'a> &'a $V : crate::FBinary<&'a AD<$V>, Output=AD<$V> >,
    {   type Output = AD<$V>;
        #[doc= " see [doc_f_binary_ad](crate::ad::binary::doc_f_binary_ad)" ]
        fn powf(self , rhs : AD<$V>) -> AD<$V> {
            (&self).powf( &rhs )
    ...
    ($V:ty, $Name:ident) => { paste::paste! {
        ...
        {   type Output = AD<$V>;
            ...
            fn [< $Name:lower >] (self , rhs : &AD<$V>) -> AD<$V> {
                ...
                //
                // op_id_pp
                let op_id_pp  = crate::op::id::[< $Name:upper _PP_OP >];
                //
                // local_key
                ...
                let (new_tape_id, new_index, new_ad_type) =
                local_key.with_borrow_mut( |tape|
                    crate::ad::binary::record_ca::<$V>
                            ( tape, &self, &rhs, op_id_pp )
                );
                //
                // result
                AD::new(new_tape_id, new_index, new_ad_type, new_value)
