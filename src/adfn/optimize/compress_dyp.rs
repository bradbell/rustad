...
impl OpHashMap {
    ...
    ///         op_seq, op_index, &first_match, map_value_in
    ///     )
    /// ```
    ///
    /// * op_seq :
    ...
    fn try_insert(
        ...
        if is_binary_op(op_id) {
            //
            // arg_0
            let arg_0 = if arg_type[0].is_dynamic() && n_dom_indext <= arg[0] {
            ...
            //
            // arg_1
            let arg_1 = if arg_type[1].is_dynamic() && n_dom_indext <= arg[1] {
            ...
            for i_arg in 0 .. arg_match.len() {
                if arg_type[i_arg].is_dynamic() && n_dom_indext <= arg[i_arg] {
            ...
            if depend.dyp[op_index + n_dom] {
                ...
                if id_all[op_index] == CALL_OP {
                    ...
                    let option = op_hash_map.try_insert(
                        &self.dyp, op_index, &first_match, map_value_in
                    );
