...
fn new_unary_op(
    ...
    //
    // new_agraph: arg_all, arg_type_all
    ...
    new_agraph.arg_type_all.push( arg_type_0 );
    //
    // old2new
...
fn new_call_op(
    ...
    let new_op_index = new_agraph.id_all.len();
    //
    // new_agraph: n_dep, id_all, arg_start
    new_agraph.n_dep += 1;
    ...
    for i_rng in 0 .. n_rng {
        if old_rng_is_dep[i_rng] {
            if new_rng_is_dep[i_rng] {
                ...
                if 0 < new_i_dep  {
                    //
                    // new_agraph: n_dep, id_all, arg_start, arg_all, arg_type_all
                    new_agraph.n_dep += 1;
                    new_agraph.id_all.push( CALL_RES_OP );
                    new_agraph.arg_start.push(new_agraph.arg_all.len() as IndexT);
                    new_agraph.arg_all.push( new_i_dep as IndexT );
...
{   //
        ...
        V : Clone + FConst + PartialEq ,
    {
        ...
        //
        // i_agraph
        let start_agraph = if n_dyp > 0 { 0 } else { 1 };
        for i_agraph in start_agraph .. 2 {
            let old_depend      : &Vec<bool>;
            let old_agraph      : &AGraph;
            let new_agraph      : &mut AGraph;
            if i_agraph == 0 {
                old_depend = &depend.dyp;
                old_agraph = &self.dyp;
                new_agraph = &mut tape.dyp;
                if trace {
                ...
                old_agraph = &self.var;
                new_agraph = &mut tape.var;
                if trace {
                    println!( "var: old_index, new_index" );
                }
            }
            //
            // new_agraph.n_dom, old2new.dyp
            let n_dom        = old_agraph.n_dom;
            new_agraph.n_dom = n_dom;
            for old_index in 0 .. n_dom { set_old2new(
                &mut old2new, i_agraph, old_index, old_index, trace
            ); }
            //
            // old_op_index, first_op
            let mut old_op_index = 0;
            while old_op_index < old_agraph.n_dep {
                //
                // old_res, op_id
                let old_res = old_op_index + old_agraph.n_dom;
                let op_id   = old_agraph.id_all[old_op_index];
                //
                // arg, arg_type
                let start =
                    old_agraph.arg_start[old_op_index] as usize;
                let end   =
                    old_agraph.arg_start[old_op_index + 1] as usize;
                let arg       = &old_agraph.arg_all[start .. end];
                let arg_type  = &old_agraph.arg_type_all[start .. end];
                //
